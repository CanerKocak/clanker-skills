...
## Delete these (real incident)

Chat-motive + restatement:

```sh
# Codex reads ~/.codex/skills. Same Grok skill set, including
# thermo-nuclear-code-quality-review and edit-the-chain.
link_tree "$ROOT/grok/skills" "$HOME/.codex/skills"
```

The `link_tree` dest already says Codex. The slash commands are the last chat
turn, not a runtime constraint.

Theater:

```md
- `~/.codex/skills/<name>` (same Grok skill set, so Codex gets `/thermo-nuclear-code-quality-review` and `/edit-the-chain`)
```
